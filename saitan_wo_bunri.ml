...
(* Section 17.8  version *)

(* 目的 : eki_t list 型のリストを受け取ったら、「最短距離最小の駅」と「最短距離最小の駅 *)
(*       以外からなるリスト」の組を返す                                          *)
(* saitan_wo_bunri : eki_t -> eki_t list -> eki_t * eki_t list *)
let saitan_wo_bunri first rest = match rest with
    [] -> (first, [])
    | first0 :: rest0 ->
    let min_eki e1 e2 = 
        match e1 with
    ...
    in let minimum_eki = List.fold_right min_eki rest first 
    in (minimum_eki, List.filter (fun x -> x <> minimum_eki) (first :: rest))
